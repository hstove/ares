...
export default ({ handleNewScore, highScore: _highScore }) => (s) => {
  let player;
  let ground;
  let ceiling;
  let boostSprite;
  ...
  let score = 0;
  // let highScore = parseInt(localStorage[`highScore${speed}`], 10) || 0;
  ...
  s.draw = function draw() {
    ...
    if (gameOver) {
      ...
      // if (score > 75) {
      //   if (Math.floor(score % framesUntilPickup) === 0) {
      //     pickups.add(newObstacle('pickup'));
      //   }
      //   if (Math.floor(score % framesUntilMine) === 0) {
      //     mines.add(newObstacle('mine'));
      //   }
      //   if (Math.floor(score % framesUntilNewMineDecay) === 0) {
      //     framesUntilMine *= mineDecay;
      //     console.log('new frames until mine', framesUntilMine);
      //   }
      // }
      const delta = score;
