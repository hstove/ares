...
export default (s) => {
  ...
  const gravity = 0.04;
  const startFramesUntilMine = 100;
  let framesUntilMine = 100;
  const framesUntilPickup = 150;
  const framesUntilNewMineDecay = 100;
  const mineWidth = 20;
  ...
  s.draw = function draw() {
    ...
    if (gameOver) {
      ...
      if (Math.floor(s.frameCount % framesUntilMine) === 0) {
        mines.add(newObstacle('mine'));
      }
      if (s.frameCount % framesUntilNewMineDecay === 0) {
